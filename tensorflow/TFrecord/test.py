...
block_size = 64
NUM_CHANNELS=1
sample_file = 'NS_samples_64_intra.txt'
label_file = 'NS_labels_64_intra.txt'
qp_file = 'NS_qps_64_intra.txt'
#read samples
...
label_None = np.zeros(5)
label_H = np.zeros(4)
label_V = np.zeros(99)
label_S = np.zeros(15)
label_HA = np.zeros(6)
label_HB = np.zeros(7)
label_VA = np.zeros(98)
label_VB = np.zeros(9)
label_H4 = np.zeros(31)
label_V4 = np.zeros(25)


K = np.array([np.size(label_None), np.size(label_H), np.size(label_V), np.size(label_S), np.size(label_HA), np.size(label_HB), np.size(label_VA), np.size(label_VB), np.size(label_H4), np.size(label_V4)] )
